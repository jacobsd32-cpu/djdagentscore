...
describe('determineRecommendation', () => {
  ...
  it('returns flagged_for_review when sybilFlag is true', () => {
    expect(determineRecommendation({ score: 90, confidence: 1.0, sybilFlag: true, gamingDetected: false })).toBe(
      'flagged_for_review',
    )
  })

  it('returns flagged_for_review when gamingDetected is true', () => {
    expect(determineRecommendation({ score: 90, confidence: 1.0, sybilFlag: false, gamingDetected: true })).toBe(
      'flagged_for_review',
    )
  })
  ...
  it('returns flagged_for_review even with high score + confidence when flags present', () => {
    ...
    expect(determineRecommendation({ score: 95, confidence: 0.99, sybilFlag: true, gamingDetected: true })).toBe(
      'flagged_for_review',
    )
  })
  ...
  it('returns insufficient_history when confidence < 0.3', () => {
    expect(determineRecommendation({ score: 80, confidence: 0.2, sybilFlag: false, gamingDetected: false })).toBe(
      'insufficient_history',
    )
  })

  it('returns insufficient_history for low confidence even with zero score', () => {
    expect(determineRecommendation({ score: 0, confidence: 0.1, sybilFlag: false, gamingDetected: false })).toBe(
      'insufficient_history',
    )
  })
  ...
  it('returns high_risk when score < 25 and confidence >= 0.5', () => {
    expect(determineRecommendation({ score: 20, confidence: 0.6, sybilFlag: false, gamingDetected: false })).toBe(
      'high_risk',
    )
  })

  it('returns high_risk at score boundary (24)', () => {
    expect(determineRecommendation({ score: 24, confidence: 0.5, sybilFlag: false, gamingDetected: false })).toBe(
      'high_risk',
    )
  })
  ...
  it('returns proceed when score >= 50 and confidence >= 0.5', () => {
    expect(determineRecommendation({ score: 50, confidence: 0.5, sybilFlag: false, gamingDetected: false })).toBe(
      'proceed',
    )
  })

  it('returns proceed for excellent wallet', () => {
    expect(determineRecommendation({ score: 95, confidence: 0.95, sybilFlag: false, gamingDetected: false })).toBe(
      'proceed',
    )
  })
  ...
  it('returns proceed_with_caution for mid-range score with sufficient confidence', () => {
    ...
    expect(determineRecommendation({ score: 35, confidence: 0.6, sybilFlag: false, gamingDetected: false })).toBe(
      'proceed_with_caution',
    )
  })

  it('returns proceed_with_caution at the 25-49 boundary', () => {
    expect(determineRecommendation({ score: 25, confidence: 0.5, sybilFlag: false, gamingDetected: false })).toBe(
      'proceed_with_caution',
    )
  })
  ...
  it('returns proceed_with_caution for decent score but borderline confidence', () => {
    ...
    expect(determineRecommendation({ score: 49, confidence: 0.4, sybilFlag: false, gamingDetected: false })).toBe(
      'proceed_with_caution',
    )
  })
  ...
  it('handles confidence exactly at 0.3 (NOT insufficient_history)', () => {
    ...
    expect(determineRecommendation({ score: 60, confidence: 0.3, sybilFlag: false, gamingDetected: false })).toBe(
      'proceed_with_caution',
    ) // confidence < 0.5 so not "proceed"
  })
  ...
  it('sybil flag overrides insufficient_history', () => {
    ...
    expect(determineRecommendation({ score: 20, confidence: 0.1, sybilFlag: true, gamingDetected: false })).toBe(
      'flagged_for_review',
  ...
  it('insufficient_history overrides high_risk', () => {
    ...
    expect(determineRecommendation({ score: 10, confidence: 0.2, sybilFlag: false, gamingDetected: false })).toBe(
      'insufficient_history',
