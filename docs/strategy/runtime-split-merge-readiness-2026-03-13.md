...
- The latest successful preview workflow run is GitHub Actions run `23040622151`.
- Preview runtime metadata reports release `0c2f6b8`, confirming the live preview matches branch head.

## What changed versus main

This branch is `10` commits ahead of `origin/main` and touches `91` files.
...
Those fixes are all on `0c2f6b8`, not just applied manually.
...
- CI hygiene: there is still a pre-existing non-blocking lint warning in `tests/jobs/anomalyDetector.test.ts` for an explicit `any`.
...
1. Treat `0c2f6b8` as the candidate promotion head.
